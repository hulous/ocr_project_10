...
class DatabaseCreationConfigurationTest {
  ...
  void dataSourceCreatesPostgresDatabaseWhenMissing() throws SQLException {
    ...
    try (MockedStatic<java.sql.DriverManager> driverManager = mockStatic(java.sql.DriverManager.class)) {
      driverManager.when(() -> java.sql.DriverManager.getConnection(
        "jdbc:postgresql://localhost:5432/postgres?ssl=false",
        "dbuser",
        "secret"
