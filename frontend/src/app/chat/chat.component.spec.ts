...
import { MessageDto } from '../core/services/chat.service';

describe('ChatComponent', () => {
  let component: ChatComponent;
  let messagesSubject: Subject<MessageDto>;
  let chatService: jasmine.SpyObj<any>;
  ...
  beforeEach(() => {
    ...
    chatService = jasmine.createSpyObj('ChatService', ['loadHistory', 'connect', 'send', 'disconnect']);
    chatService.messages$ = messagesSubject.asObservable();
    chatService.loadHistory.and.returnValue(of([historyMessage]));
    const route = { snapshot: { paramMap: { get: () => 'conversation-1' } } } as unknown as ActivatedRoute;
    component = new ChatComponent(route, chatService);
  });

  it('loads history, connects, and accepts messages for the active conversation', () => {
    component.ngOnInit();
    messagesSubject.next({ ...historyMessage, id: 'live-1', content: 'Live message' });
    messagesSubject.next({ ...historyMessage, conversationId: 'other', id: 'ignored' });
