...
(globalThis as any).jasmine = {
  ...
  createSpyObj: (name: string, methods: string[]) =>
    Object.fromEntries(methods.map((method) => [method, createJasmineSpy(`${name}.${method}`)])),
  objectContaining: (value: unknown) => expect.objectContaining(value as object),
  any: (constructor: unknown) => expect.any(constructor as any),
};

function createJasmineSpy(name: string) {
  const spy = vi.fn().mockName(name) as any;
  spy.and = {
